...
inline static void sample_info_push(Ijk_VideoToolBox_Opaque* context)
{
    ...
    if (sample_info->is_decoding) {
        av_log(NULL, AV_LOG_WARNING, "%s, reallocate sample in decoding %d -> %d /%d\n", __FUNCTION__,
...
inline static void sample_info_recycle(Ijk_VideoToolBox_Opaque* context, sample_info *sample_info)
{
    ...
    if (sample_info->is_decoding) {
        ...
        av_log(NULL, AV_LOG_WARNING, "%s, multiple frames in same sample %d / %d\n", __FUNCTION__,
               sample_info->sample_id,
               context->sample_info_id_generator);
    }
...
static void QueuePicture(Ijk_VideoToolBox_Opaque* ctx) {
    ...
    if (true == GetVTBPicture(ctx, &picture)) {
        ...
        av_log(NULL, AV_LOG_INFO, "Get Picture failure!!!\n");
    }
}
                       ...
                       CMTime presentationDuration)
{
    @autoreleasepool {
        ...
        if (!sample_info->is_decoding) {
            av_log(NULL, AV_LOG_DEBUG, "VTB: frame out of date: id=%d\n", sample_info->sample_id);
            goto failed;
        }

        newFrame = (sort_queue *)mallocz(sizeof(sort_queue));
        if (!newFrame) {
            av_log(NULL, AV_LOG_ERROR, "VTB: create new frame fail: out of memory\n");
            goto failed;
        }
        ...
        if (imageBuffer == NULL) {
            av_log(NULL, AV_LOG_INFO, "imageBuffer null\n");
            goto failed;
        }
        ...
        if (format_type != kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange) {
            av_log(NULL, AV_LOG_INFO, "format_type error \n");
            goto failed;
        }
        if (kVTDecodeInfo_FrameDropped & infoFlags) {
            av_log(NULL, AV_LOG_INFO, "droped\n");
            goto failed;
        }

        if (ctx->new_seg_flag) {
            av_log(NULL, AV_LOG_INFO, "new seg process!!!!");
...
static VTDecompressionSessionRef vtbsession_create(Ijk_VideoToolBox_Opaque* context)
{
    ...
    av_log(NULL, AV_LOG_INFO, "after scale width %d height %d \n", width, height);
...
static inline void DuplicatePkt(Ijk_VideoToolBox_Opaque* context, const AVPacket* pkt) {
    if (context->m_buffer_deep >= MAX_PKT_QUEUE_DEEP) {
        ...
        av_log(NULL, AV_LOG_ERROR, "m_buffer overflow\n");
...
static int decode_video(Ijk_VideoToolBox_Opaque* context, AVCodecContext *avctx, AVPacket *avpkt, int* got_picture_ptr)
{
    ...
    bool     isAnnexB = context->ffp->isAnnexB;
    if (!avpkt || !avpkt->data) {
        return 0;
    }
        ...
        if (size_data && size_data_size > 7) {
            ...
            if (ret < 0) {
                ...
                if (context->codecpar->width  != new_avctx->width ||
                    ...
                    context->codecpar->profile != new_avctx->profile ||
                    context->codecpar->level != new_avctx->level) {
        ...
        if ((context->m_buffer_deep > 0) &&
           ff_avpacket_i_or_idr(&context->m_buffer_packet[0], context->idr_based_identified, isAnnexB, context->is_hevc) == true ) {
            for (int i = 0; i < context->m_buffer_deep; i++) {
                ...
                av_log(NULL, AV_LOG_ERROR, "recovery success!!!!\n");
            }
        } else {
            context->recovery_drop_packet = true;
            ret = -1;
        ...
        if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
            ...
            do {
                ...
                if (ffp_is_flush_packet(&pkt)) {
                    ...
                    context->serial += 1;
                    d->finished = 0;
                    av_log(NULL, AV_LOG_INFO, "flushed last keyframe pts %lld \n",d->pkt.pts);
...
static int vtbformat_init(VTBFormatDesc *fmt_desc, AVCodecParameters *codecpar)
{
    ...
    if (extrasize < 7 || extradata == NULL) {
        av_log(NULL, AV_LOG_INFO, "%s - avcC or hvcC atom data too small or missing", __FUNCTION__);
        goto fail;
    }
...
//                }
        if (level == 0 && sps_level > 0)
            level = sps_level;
                ...
                if (profile == FF_PROFILE_H264_MAIN && level == 32 && fmt_desc->max_ref_frames > 4) {
                    av_log(NULL, AV_LOG_ERROR, "%s - Main@L3.2 detected, VTB cannot decode with %d ref frames", __FUNCTION__, fmt_desc->max_ref_frames);
                    goto fail;
                }
        ...
        if (fmt_desc->fmt_desc == NULL) {
            goto fail;
                ...
                av_log(NULL, AV_LOG_INFO, "%s - using avcC atom of size(%d), ref_frames(%d)", __FUNCTION__, extrasize, fmt_desc->max_ref_frames);
            } else {
            ...
            av_log(NULL, AV_LOG_INFO, "%s - invalid avcC atom data", __FUNCTION__);
            goto fail;
        }
    }
    ...
    av_log(NULL, AV_LOG_INFO, "m_max_ref_frames %d \n", fmt_desc->max_ref_frames);
...
Ijk_VideoToolBox_Opaque* videotoolbox_async_create(FFPlayer* ffp, AVCodecContext* avctx)
{
    ...
    if (ret) {
        av_log(NULL, AV_LOG_WARNING, "%s - videotoolbox can not exists twice at the same time", __FUNCTION__);
        return NULL;
    }
