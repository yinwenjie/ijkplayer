...
static int decoder_open(FFPlayer *ffp, AVCodecContext ** pavctx)
{
    ...
    if(codec->capabilities & AV_CODEC_CAP_DR1)
        avctx->flags |= CODEC_FLAG_EMU_EDGE;
#endif
    if (!av_dict_get(ffp->codec_opts, "threads", NULL, 0))
        av_dict_set(&ffp->codec_opts, "threads", "auto", 0);
    if (stream_lowres)
        av_dict_set_int(&ffp->codec_opts, "lowres", stream_lowres, 0);
    if (avctx->codec_type == AVMEDIA_TYPE_VIDEO || avctx->codec_type == AVMEDIA_TYPE_AUDIO)
        av_dict_set(&ffp->codec_opts, "refcounted_frames", "1", 0);
    if ((ret = avcodec_open2(avctx, codec, &ffp->codec_opts)) < 0) {
        goto fail;
    }
    if ((t = av_dict_get(ffp->codec_opts, "", NULL, AV_DICT_IGNORE_SUFFIX))) {
        av_log(NULL, AV_LOG_ERROR, "Option %s not found.\n", t->key);
    }
...
static AVCodecContext * create_video_decoder_by_extradata (FFPlayer *ffp, char *extradata_enc) {
    ...
    avctx->extradata_size = av_base64_decode(avctx->extradata, extradata_enc, extradata_size);
    ...
    ffp->video_extradata_guess = av_malloc(avctx->extradata_size);
    memcpy(ffp->video_extradata_guess, avctx->extradata, avctx->extradata_size);
    ffp->video_extradata_size_guess = avctx->extradata_size;
    ...
    // framerate = time_scale / (num_units_in_tick * ( 1 + nuit_field_based_flag ))
    av_reduce(&avctx->framerate.num, &avctx->framerate.den, sps->time_scale,
              sps->num_units_in_tick * (H264_NUIT_FIELD_BASED_FLAG + 1), INT_MAX);
...
static AVCodecContext * create_audio_decoder_by_extradata (FFPlayer *ffp, char *extradata_enc) {
    ...
    //Init Audio Stream
    if ((pCodec = avcodec_find_decoder(AV_CODEC_ID_AAC)) == NULL)
        return NULL;

    if ((avctx = avcodec_alloc_context3(pCodec)) == NULL)
        return NULL;
    ...
    ffp->audio_extradata_guess = av_malloc(avctx->extradata_size);
    memcpy(ffp->audio_extradata_guess, avctx->extradata, avctx->extradata_size);
    ffp->audio_extradata_size_guess = avctx->extradata_size;
...
#define    ASYNC_ERROR_INIT_VCODEC          -5
#define    ASYNC_ERROR_INIT_ACODEC          -6
#define    ASYNC_ERROR_VCODEC_EXTRADATA     -7
#define    ASYNC_ERROR_ACODEC_EXTRADATA     -8
#define    ASYNC_ERROR_UNKNOWN              -9

#define    ASYNC_INIT_VCODEC_FAIL           (1 << 0)
#define    ASYNC_INIT_ACODEC_FAIL           (1 << 1)
...
static int check_decoders(FFPlayer *ffp) {
    VideoState *is = ffp->is;

    while(!is->initialized_decoder)
        SDL_Delay(5);

    if (is->async_init_flags & ASYNC_INIT_VCODEC_FAIL)
        return ASYNC_ERROR_INIT_VCODEC;

    if (is->async_init_flags & ASYNC_INIT_ACODEC_FAIL)
        return ASYNC_ERROR_INIT_ACODEC;
...
retry_info:
    ...
    if (!ffp->async_init_decoder || !ffp->use_extradata) {
        ...
        is->last_video_stream    = is->video_stream;
        is->last_audio_stream    = is->audio_stream;
        is->last_subtitle_stream = is->subtitle_stream = -1;
    }
    ...
    if (ffp->async_init_decoder && ffp->use_extradata) {
        ...
        av_dict_set_int(&ffp->format_opts, "auto_convert", 0, 0);
    }

    if (ffp->iformat_name && strlen(ffp->iformat_name))
    ...
    if (!ffp->async_init_decoder || !ffp->use_extradata) {
        ...
        av_dict_set_int(&ic->metadata, "nb-streams", is->nb_streams_guess, 0);
        ret = ASYNC_ERROR_NONE;
        if (((find_stream_error = avformat_find_stream_info(ffp->is->ic, NULL)) < 0) ||
            ((ret               = check_streams(ffp, is->nb_streams_guess))     < 0) ||
    ...
    if (!ffp->async_init_decoder || !ffp->use_extradata) {
        ...
        st_index[AVMEDIA_TYPE_VIDEO]    = is->video_stream;
        st_index[AVMEDIA_TYPE_AUDIO]    = is->audio_stream;
        st_index[AVMEDIA_TYPE_SUBTITLE] = AVERROR_STREAM_NOT_FOUND;
    }
    ...
    // copy pre-init codec to each stream
    if (ffp->async_init_decoder && ffp->use_extradata) {
        copy_stream_info(is);
        if (ffp->max_fps >= 0) {
            if(is->video_st->avg_frame_rate.den && is->video_st->avg_frame_rate.num) {
        ...
        if (is->is_video_high_fps) {
            is->viddec.avctx->skip_frame       = FFMAX(is->viddec.avctx->skip_frame, AVDISCARD_NONREF);
...
FF_DISABLE_DEPRECATION_WARNINGS
                    ...
                    ic->streams[is->audio_stream]->codecpar->bit_rate =
...
static int guess_decoders(FFPlayer *ffp) {
    int ret = 0;
    VideoState *is = ffp->is;
    is->nb_streams_guess = 0;


    /*
     * Don't support video-only or audio-only video
     */
    if ((!ffp->video_extradata || !strlen(ffp->video_extradata)) ||
        (!ffp->audio_extradata || !strlen(ffp->audio_extradata))) {
        return ASYNC_ERROR_NO_EXTRADATA;
    }

    if (ffp->video_extradata && strlen(ffp->video_extradata)) {
        if ((is->viddec.avctx = create_video_decoder_by_extradata(ffp, ffp->video_extradata)) == NULL) {
            return ASYNC_ERROR_VCODEC_EXTRADATA;
        }
        is->video_stream = is->nb_streams_guess++;
    } else {
        is->video_stream = -1;
    }

    if (ffp->audio_extradata && strlen(ffp->audio_extradata)) {
        if ((is->auddec.avctx = create_audio_decoder_by_extradata(ffp, ffp->audio_extradata)) == NULL) {
            avcodec_free_context(&is->viddec.avctx);
            return  ASYNC_ERROR_ACODEC_EXTRADATA;
        }
        is->audio_stream = is->nb_streams_guess++;
    } else {
        is->audio_stream = -1;
    }
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    if (ffp->async_init_decoder && ffp->use_extradata) {
        if ((ffp->async_error_code = guess_decoders(ffp)) < 0) {
            ...
            av_dict_copy(&is->orig_format_opts, ffp->format_opts, 0);
            av_dict_copy(&is->orig_codec_opts, ffp->codec_opts, 0);
            av_dict_copy(&is->orig_sws_dict, ffp->sws_dict, 0);
            /*av_dict_copy(&is->orig_player_opts, ffp->orig_player_opts, 0);*/
            av_dict_copy(&is->orig_swr_opts, ffp->swr_opts, 0);
            av_dict_copy(&is->orig_swr_preset_opts, ffp->swr_preset_opts, 0);
        }
    }
    ...
    if (ffp->async_init_decoder) {
        if (ffp->use_extradata) {
            /* open the decoders*/
            av_log(ffp, AV_LOG_ERROR, "async init decoder begin\n");

            if (is->video_stream >= 0 && decoder_open(ffp, &ffp->is->viddec.avctx) < 0) {
                av_log(NULL, AV_LOG_ERROR, "open video codec fail\n");
                is->async_init_flags |= ASYNC_INIT_VCODEC_FAIL;
            }

            if (is->audio_stream >= 0 && decoder_open(ffp, &ffp->is->auddec.avctx) < 0) {
...
int ffp_get_video_rotate_degrees(FFPlayer *ffp)
{
    ...
    int theta;
    if (ffp->async_init_decoder && ffp->use_extradata) {
        theta = ffp->mediacodec_rotate_degrees;
    } else {
        theta = abs((int)((int64_t)round(fabs(get_rotation(is->video_st))) % 360));
    }
    switch (theta) {
        case 0:
        case 90:
